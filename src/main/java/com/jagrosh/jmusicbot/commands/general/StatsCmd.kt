...
class StatsCmd(bot: Bot) : Command() {
    override fun execute(event: CommandEvent) {
        ...
        val duration: Long = ManagementFactory.getRuntimeMXBean().getUptime()
        ...
        var uptime: String? = ((if (years == 0L) "" else "**$years**y ") + (if (months == 0L) "" else "**$months**mo ") + (if (days == 0L) "" else "**$days**d ") + (if (hours == 0L) "" else "**$hours**h ")
